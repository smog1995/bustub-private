...
auto LockManager::LockTable(Transaction *txn, LockMode lock_mode, const table_oid_t &oid) -> bool {
  ...
  if (table_lock_map_.find(oid) == table_lock_map_.end()) {
    std::cout << "table" << oid << "doesn't exists in lock_map, so it needs create a map from oid to queue"
              << std::endl;
  ...
  std::cout << "队列中请求数量:" << lock_request_queue->request_queue_.size() << std::endl;
  for (auto ele = lock_request_queue->request_queue_.begin(); ele != lock_request_queue->request_queue_.end();) {
    std::cout << "detect whether upgrade lock or not";
    if ((*ele)->txn_id_ == txn->GetTransactionId()) {
      std::cout << "DETECTED UPGRADE_lock! ";
      ...
      std::cout << "ele's lock_mode:" << static_cast<int>((*ele)->lock_mode_)
                << " lock_mode:" << static_cast<int>(lock_mode) << std::endl;
      //  如果升级不兼容，抛出异常
...
void LockManager::DeleteTableLockInTransaction(Transaction *txn, LockMode lock_mode, table_oid_t oid) {
  ...
  switch (lock_mode) {
    case LockMode::INTENTION_SHARED:
      std::cout << txn->GetTransactionId() << "解意向读锁在表" << oid;
      txn->GetIntentionSharedTableLockSet()->erase(oid);
      break;
    case LockMode::SHARED:
      std::cout << txn->GetTransactionId() << "解读锁在表" << oid;
      txn->GetSharedTableLockSet()->erase(oid);
      break;
    case LockMode::SHARED_INTENTION_EXCLUSIVE:
      std::cout << txn->GetTransactionId() << "解意向读写锁在表" << oid;
      txn->GetSharedIntentionExclusiveTableLockSet()->erase(oid);
      break;
    case LockMode::EXCLUSIVE:
      std::cout << txn->GetTransactionId() << "解写锁在表" << oid;
      txn->GetExclusiveTableLockSet()->erase(oid);
      break;
    case LockMode::INTENTION_EXCLUSIVE:
      std::cout << txn->GetTransactionId() << "解意向写锁在表" << oid;
...
void LockManager::InsertTableLockInTransaction(Transaction *txn, LockMode lock_mode, table_oid_t oid) {
  ...
  switch (lock_mode) {
    case LockMode::INTENTION_SHARED:
      std::cout << "(状态)" << txn->GetTransactionId() << "加意向读锁在表" << oid;
      txn->GetIntentionSharedTableLockSet()->insert(oid);
      break;
    case LockMode::SHARED:
      std::cout << "(状态)" << txn->GetTransactionId() << "加读锁在表" << oid;
      txn->GetSharedTableLockSet()->insert(oid);
      break;
    case LockMode::SHARED_INTENTION_EXCLUSIVE:
      std::cout << "(状态)" << txn->GetTransactionId() << "加意向读写锁在表" << oid;
      txn->GetSharedIntentionExclusiveTableLockSet()->insert(oid);
      std::cout << "(状态)" << txn->GetTransactionId();
      break;
    case LockMode::EXCLUSIVE:
      std::cout << "(状态)" << txn->GetTransactionId() << "加写锁在表" << oid;
      txn->GetExclusiveTableLockSet()->insert(oid);
      std::cout << "|||写锁数量:" << txn->GetExclusiveTableLockSet()->size();
      break;
    case LockMode::INTENTION_EXCLUSIVE:
      std::cout << "(状态)" << txn->GetTransactionId() << "加意向写锁在表" << oid;
...
auto LockManager::GrantLock(LockRequestQueue *lock_request_queue, LockMode lock_mode, txn_id_t txn_id) -> bool {
  ...
  //  第三步，不存在锁升级，则根据优先级来决定是否授予锁,找到第一个waiting请求
  for (auto &ele : lock_request_queue->request_queue_) {
    //  找到第一个waiting请求,而且该请求不是当前事务，同时不兼容上锁，返回false
    if (!ele->granted_) {
      if (ele->txn_id_ != txn_id && !compatable_lock_[static_cast<int>(lock_mode)][static_cast<int>(ele->lock_mode_)]) {
        return false;
      }
      break;
    }
...
auto LockManager::UnlockTable(Transaction *txn, const table_oid_t &oid) -> bool {
  ...
  for (auto ele = request_lock_queue->request_queue_.begin(); ele != request_lock_queue->request_queue_.end(); ele++) {
    if ((*ele)->txn_id_ == txn->GetTransactionId()) {
      ...
      if (TransactionManager::GetTransaction((*ele)->txn_id_)->GetState() != TransactionState::ABORTED) {
...
auto LockManager::LockRow(Transaction *txn, LockMode lock_mode, const table_oid_t &oid, const RID &rid) -> bool {
  ...
  std::cout << "lock row finish" << std::endl;
  return true;
}
...
auto LockManager::RemoveAbortedEdge(txn_id_t aborted_txn_id) -> void {
  std::unique_lock<std::mutex> lock(waits_for_latch_);
  for (auto vertex_iterator = waits_for_.begin(); vertex_iterator != waits_for_.end();) {
    if (vertex_iterator->first == aborted_txn_id) {
      vertex_iterator->second.clear();
    } else {
      for (auto edge_iterator = vertex_iterator->second.begin(); edge_iterator != vertex_iterator->second.end();) {
        if (*edge_iterator == aborted_txn_id) {
          edge_iterator = vertex_iterator->second.erase(edge_iterator);
        } else {
          edge_iterator++;
        }
      }
...
void LockManager::DepthFirstSearch(txn_id_t txn_id, bool &cycle_flag, txn_id_t *youngest_txn_id) {
  if (onpath_[txn_id]) {
    cycle_flag = true;
    if (*youngest_txn_id < txn_id) {
      *youngest_txn_id = txn_id;
    }
    return;
  }
  ...
  for (auto &ele : graph_[txn_id]) {
    DepthFirstSearch(ele, cycle_flag, youngest_txn_id);
    if (cycle_flag) {
      if (*youngest_txn_id < txn_id) {
        *youngest_txn_id = txn_id;
      }
...
auto LockManager::HasCycle(txn_id_t *txn_id) -> bool {
  *txn_id = INVALID_TXN_ID;
  ...
  for (auto &ele : waits_for_) {
    visited_[ele.first] = onpath_[ele.first] = false;
    vertex_.push_back(ele.first);
  }
  std::sort(vertex_.begin(), vertex_.end(), std::less<>());
  for (auto &ele : vertex_) {
    if (!graph_[ele].empty()) {
      DepthFirstSearch(ele, cycle_flag, txn_id);
    }
  }
  ...
  while (enable_cycle_detection_) {
    ...
    {  // TODO(students): detect deadlock
      ...
      if (HasCycle(&youngest_txn_id)) {
        ...
        RemoveAbortedEdge(youngest_txn_id);
